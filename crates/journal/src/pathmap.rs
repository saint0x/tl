...
use ahash::AHashMap;
use anyhow::Result;
use core::{Sha1Hash, Entry, EntryKind, Tree};
...
impl PathMap {
    ...
    pub fn new(root_tree: Sha1Hash) -> Self {
        Self {
            root_tree,
    ...
    pub fn from_tree(tree: &Tree, root_tree: Sha1Hash) -> Self {
    ...
    /// Save PathMap to disk (PMV2 format - Git-compatible)
    ///
    /// Format:
    /// - magic: "PMV2" (4 bytes)
    /// - root_tree: Sha1Hash (20 bytes)
    /// - entry_count: u32 (4 bytes)
    /// - entries (sorted by path):
    ///   - path_len: u16
    ///   - path_bytes: [u8; path_len]
    ///   - kind: u8 (0=file, 1=executable, 2=symlink, 3=tree)
    ///   - mode: u32
    ///   - blob_hash: [u8; 20]
    pub fn save(&self, path: &Path) -> Result<()> {
        const MAGIC: &[u8] = b"PMV2";
        ...
        // Write root_tree hash (20 bytes for SHA-1)
        ...
        for (path_bytes, entry) in sorted_entries {
            ...
            // Kind (u8) - updated for Git tree format
            let kind_byte = match entry.kind {
                EntryKind::File => 0u8,
                EntryKind::ExecutableFile => 1u8,
    ...
    /// Load PathMap from disk (supports PMV1 and PMV2 formats)
    pub fn load(path: &Path) -> Result<Self> {
        ...
        if bytes.len() < 4 {
            anyhow::bail!("Invalid PathMap: file too short");
        }

        // Check magic and dispatch to appropriate loader
        match &bytes[0..4] {
            b"PMV2" => Self::load_v2(&bytes),
            b"PMV1" => anyhow::bail!("PMV1 format no longer supported - please reinitialize repository"),
            _ => anyhow::bail!("Invalid PathMap magic: {:?}", &bytes[0..4]),
        }
    }

    /// Load PMV2 format (Git-compatible with SHA-1)
    fn load_v2(bytes: &[u8]) -> Result<Self> {
        if bytes.len() < 28 {
            anyhow::bail!(
                "Invalid PMV2: file too short (expected at least 28 bytes, got {})",
                bytes.len()
            );
        }

        // Read root_tree hash (20 bytes)
        let mut root_tree_bytes = [0u8; 20];
        root_tree_bytes.copy_from_slice(&bytes[4..24]);
        let root_tree = Sha1Hash::from_bytes(root_tree_bytes);

        // Read entry count
        let entry_count = u32::from_le_bytes([bytes[24], bytes[25], bytes[26], bytes[27]]) as usize;
        ...
        for i in 0..entry_count {
            if offset + 2 > bytes.len() {
                anyhow::bail!("Invalid PMV2: incomplete entry {} at offset {}", i, offset);
            }
            ...
            if offset + path_len > bytes.len() {
                anyhow::bail!("Invalid PMV2: path too long for entry {} at offset {}", i, offset);
            }
            ...
            if offset + 1 + 4 + 20 > bytes.len() {
                anyhow::bail!("Invalid PMV2: incomplete entry metadata at offset {}", offset);
            }

            // Read kind (updated for Git tree format)
            ...
            // Read blob hash (20 bytes)
            let mut hash_bytes = [0u8; 20];
            hash_bytes.copy_from_slice(&bytes[offset..offset + 20]);
...
mod tests {
    use super::*;
    use core::{Entry, EntryKind, Tree, hash::git::hash_blob};
